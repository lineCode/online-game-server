...
WebApp.controller('UploadController', ['$scope', '$rootScope', 'HttpService', function($scope, $rootScope, HttpService) {
  $scope.baseUri = getBaseUri(document.URL);
  
  $scope.releases = null;
  $scope.requestPara = null;

  HttpService.request($scope.baseUri + 'child_admin', 'POST', JSON.stringify(
    {
    ...
    HttpService.request($scope.baseUri + 'child_admin', 'POST', JSON.stringify(
      ...
      function(res) {
        if (res.succeeded == true) {
          ...
          toastr.success('활성화되였습니다.', '성공');
        } else {
          toastr.error('실패되였습니다.', '실패');
        }
      });
  };
