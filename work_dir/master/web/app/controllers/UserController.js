WebApp.controller('UserController', ['$scope', '$rootScope', 'HttpService', function($scope, $rootScope, HttpService) {
  $scope.user_ids = [];
  ...
  HttpService.request($scope.baseUri + 'user/list', 'POST', '{}').then(function(res) {
  ...
  $scope.view_detail = function(userid) {
  	HttpService.request($scope.baseUri + 'user/info', 'POST', '{"id":' + userid + '}')
  		.then(function(res){
  ...
  $scope.create_user = function() {
    var x = '{"login_name":"' + $scope.new_user.name + '",' +
                           '"active":' + $scope.new_user.active + '}';
    var x = {"login_name":$scope.new_user.name,
              "active":Boolean($scope.new_user.active)} 
    $scope.requestPara = JSON.stringify(x);
    HttpService.request($scope.baseUri + 'user/create', 'POST', $scope.requestPara).then(
      function(res) {
        if (res.succeeded == true) {
  ...
  $scope.modify_user = function() {
    $scope.requestPara = '{ "id":' + $scope.selected_user.id + ',' +
                            '"login_name":"' + $scope.selected_user.login_name + '",' +
                            '"active":' + $scope.selected_user.active + '}'; 
    HttpService.request($scope.baseUri + 'user/modify', 'POST', $scope.requestPara).then(
      function(res) {
        if (res.succeeded == true) {
  ...
  $scope.set_password = function() {
    $scope.password_hash = CryptoJS.SHA256($scope.set_password.password).toString().toUpperCase();
    $scope.requestPara = '{"id":' + $scope.set_password.user_id + ',' +
                           '"password_hash":"' + $scope.password_hash + '"}'; 
    HttpService.request($scope.baseUri + 'user/set_password_hash', 'POST', $scope.requestPara).then(
      function(res) {
        if (res.succeeded == true) {
