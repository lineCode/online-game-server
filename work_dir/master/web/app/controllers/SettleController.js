...
'use strict';
WebApp.controller('SettleController', ['$scope', '$rootScope', 'HttpService', function($scope, $rootScope, http) {
  $scope.settle = [];
  $scope.baseUri = getBaseUri(document.URL);

  var dt = new Date();

  $scope.startDate = dt.getFullYear() + "-" + stylizer(dt.getMonth()+1) + "-" + stylizer(1);
  $scope.endDate = dt.getFullYear() + "-" + stylizer(dt.getMonth()+1) + "-" + stylizer(dt.getDate());
  ...
  $scope.get_detail = function(usr_id) {
    var data = {"user" : usr_id,
                "start_date": $scope.settle.startDate,
                "end_date" : $scope.settle.endDate};
  	http.request($scope.baseUri + 'settle/by_user', 'POST', JSON.stringify(data))
  		.then(function(res){
  ...
  $scope.get_total = function() {
    var data = {"start_date": $scope.settle.startDate,
                "end_date" : $scope.settle.endDate};
  	http.request($scope.baseUri + 'settle/total', 'POST', JSON.stringify(data)).then(
  		function(res) {
  			if (res.succeeded == true) {
          $scope.rows = res.rows;
